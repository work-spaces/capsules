...
def _declare_capsule(domain, owner, repo, version, rev = None, deps = []):
...
def _declare_github_capsule(owner, repo, version, rev = None, deps = []):
    return _declare_capsule("github.com", owner, repo, version, rev, deps)
...
GITHUB_COM_GNOME_LIBXML_V2 = _declare_github_capsule("gnome", "libxml2", "2.13.5", deps = [FTP_GNU_ORG_LIBICONV_V1])
